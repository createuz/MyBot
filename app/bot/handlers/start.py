...
async def start_handler(message: types.Message, **data):
    ...
    # 1) Try Redis-only (fast path, no DB session created)
    lang = await redis_get_lang(redis, tg_id)
    if lang:
        logger.debug(f"start: redis hit {tg_id} -> {lang}")
    ...
    if lang:
        logger.debug(f"start: db hit {tg_id} -> {lang}")
        await message.answer(t(lang, "greeting"))
        return

    # 3) Not found -> ensure user record (default en), then ask for language
    try:
        await ensure_user_exists(session=db, chat_id=tg_id, username=username,
                                 first_name=first_name, is_premium=is_premium, default_lang="en")
        logger.info(f"start: ensured user exists default en for {tg_id}")
    except Exception as e:
        logger.exception(f"start: ensure_user_exists failed for {tg_id}: {e}")
